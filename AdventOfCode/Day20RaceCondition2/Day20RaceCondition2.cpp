// Day20RaceCondition2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
// 
...
static bool inside(int row, int col, int height, int width) {
...
static std::vector<std::vector<int>> bfs(std::pair<int, int> start, std::vector<std::vector<bool>>& racetrack) {
	...
	for (size_t idq = 0; idq < queue.size(); idq++) {
		...
		for (std::pair<int, int> dir : dirs) {
			...
			if (inside(static_cast<int>(r2), static_cast<int>(c2), static_cast<int>(racetrack.size()), static_cast<int>(racetrack[0].size())) && d[r2][c2] == INT_MAX) {
				d[r2][c2] = d[r][c] + 1;
...
int main()
{
	...
	int x = 0;
	int y = 0;
	long long resultCheatsWithLeastXPicoseconds = 0;
	const int picoSeconds = 100;
	const int allowedCheats = 20;
	...
	for (int idyHeight = 0; idyHeight < static_cast<int>(racetrack.size()); idyHeight++) {
		for (int idxWidth = 0; idxWidth < static_cast<int>(racetrack[0].size()); idxWidth++) {

			if (racetrack[static_cast<size_t>(idyHeight)][static_cast<size_t>(idxWidth)] != false) {
				continue;
			}

			for (int idyHeight2 = std::max(0, idyHeight - allowedCheats); idyHeight2 <= std::min(static_cast<int>(racetrack.size()) - 1, idyHeight + allowedCheats); idyHeight2++) {
				for (int idxWidth2 = std::max(0, idxWidth - allowedCheats); idxWidth2 <= std::min(static_cast<int>(racetrack[0].size()) - 1, idxWidth + allowedCheats); idxWidth2++) {
					if (std::abs(idyHeight- idyHeight2)+std::abs(idxWidth- idxWidth2) <= allowedCheats) {
						if (inside(idyHeight2, idxWidth2, static_cast<int>(racetrack.size()), static_cast<int>(racetrack[0].size())) && racetrack[static_cast<size_t>(idyHeight2)][static_cast<size_t>(idxWidth2)] != true) {
							int here = forwardPath[static_cast<size_t>(idyHeight)][static_cast<size_t>(idxWidth)] + backwardPath[static_cast<size_t>(idyHeight2)][static_cast<size_t>(idxWidth2)] + std::abs(idyHeight - idyHeight2) + std::abs(idxWidth - idxWidth2);
