// Day08ResonantCollinearity2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	while (inputfile.get(chInput)) {
		...
		if (chInput == '\n') {
			rows++;
			width = static_cast<size_t>(cols - 1);
			cols = 0;
		}


		height = static_cast<size_t>(rows + 1);
	}
	...
	for (auto mappoint : map) {
		...
		if (ch != '#') {
			...
			while (itFindCh != map.end()) {
				...
				while ((newX >= 0 && newX < static_cast<int>(width)) && (newY >= 0 && newY < static_cast<int>(height))) {
					if (mapAntiNodes.find({ newX, newY }) == mapAntiNodes.end() /* && (newX >= 0 && newX < width) && (newY >= 0 && newY < height)*/)
						mapAntiNodes[{newX, newY}] = '#';


					newX -= (diffX); // > 0 ? x - diffX : x + std::abs(diffX);
					newY -= (diffY); // > 0 ? y - diffY : y + std::abs(diffY);
				}
